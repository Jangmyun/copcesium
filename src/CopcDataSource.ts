...
export class CopcDataSource {
  ...
  private _recordStage(stage: StageName, startedAt: number): void {
    ...
    try {
      performance.measure(`copcesium ${stage}`, { start: startedAt, end });
    } catch {
      // User Timing is unavailable (or its buffer full) in some embedders;
      // the samples above remain the source of truth for `stats`.
    }
  }
